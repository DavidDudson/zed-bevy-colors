...
`crates/bevy-color-lsp` uses `#![deny(missing_docs)]` — every `pub`
item must have a `///` doc comment. The `lint.yml` CI job runs
`RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps` to enforce
this. The `zed-extension` crate uses `#![allow(missing_docs)]` because its
only public symbol is generated by the `zed::register_extension!`
macro and cannot carry a doc comment; once authored public items
exist, flip to `warn` or `deny`.
